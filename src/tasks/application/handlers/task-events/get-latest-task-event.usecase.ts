...
@Injectable()
export class GetLatestTaskEventUseCase {
  private readonly logger = new Logger(GetLatestTaskEventUseCase.name);
  ...
  async execute(taskId: string, userId: string) {
    ...
    const event = await this.taskEvents.findLatest(taskId, userId);
    if (!event) throw new NotFoundException('TASK_EVENT_NOT_FOUND');

    this.logger.log(
      `Latest task event fetched for task ${taskId} and user ${userId}`,
    );
    return event;
  }
}
